...
export class Empchart1Component implements OnInit {
  ...
  openYearSelectModal(): void {
    Swal.fire({
      title: 'Select Year',
      input: 'select',
      inputOptions: {
        '': 'All Years',
        ...this.getAvailableYearOptions()
      },
      inputValue: this.selectedYear,
  ...
  getAvailableYearOptions(): { [key: string]: string } {
  ...
  getAvailableYears(): string[] {
    ...
    this.leaveDetails.forEach(leave => {
      ...
      yearsSet.add(leaveStartDate.getFullYear().toString());
    });
  ...
  onYearSelect(): void {
    this.filterLeaveDetailsByYear();
  }

  filterLeaveDetailsByYear(): void {
    if (this.selectedYear && this.leaveDetails.length > 0) {
      this.filteredLeaveDetails = this.leaveDetails.filter(leave => {
        const leaveStartDate = new Date(leave.startDate);
        return leaveStartDate.getFullYear().toString() === this.selectedYear;
      });
    } else {
      this.filteredLeaveDetails = [...this.leaveDetails];
    }

    const filteredLeaveTypesData = this.extractLeaveTypesData(this.filteredLeaveDetails);

    this.casualLeaveData = filteredLeaveTypesData.filter(item => item.leaveType === 'casual');
    this.sickLeaveData = filteredLeaveTypesData.filter(item => item.leaveType === 'sick');
    this.earnedLeaveData = filteredLeaveTypesData.filter(item => item.leaveType === 'earned');
  ...
  renderDoughnutChart(data: any[], chartId: string, totalLeaves: number): void {
    ...
    if (chartContainer) {
      ...
      if (data.length === 0) {
        data.push({ leaveType: 'unknown', value: 0 });
      }
      ...
      svg
        ...
        valuesContainer.innerHTML = `
        <div>
          <h4 style="color: #252C58" ;>${chartName}</h4>
